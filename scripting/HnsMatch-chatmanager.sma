...
#define PLUGIN "Hns Match Chat"
#define VERSION "DEV"
#define AUTHOR "Mistrick(main), OpenHNS"
...
public FormatMessage(sender, CsTeams:sender_team, channel, name_color, chat_color, time_code[], name[], message[]) {
    ...
    /* HNS MATCH PTS */
    len += formatex(text[len], charsmax(text) - len, "^1[^3%s^1] ", get_skill_player(hns_pts_get_player_info(sender, pts)));
    /* HNS MATCH PTS */
